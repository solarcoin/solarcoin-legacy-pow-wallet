...
// Copyright (c) 2013-2014 SolarCoin developers
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if (coins.IsCoinBase()) {
                if (nSpendHeight - coins.nHeight < COINBASE_MATURITY)
                    return state.Invalid(error("CheckInputs() : coin.nHeight = (%d), spendHeight(%d), tried to spend coinbase at depth %d", coins.nHeight,nSpendHeight,  nSpendHeight - coins.nHeight));
            }
...
{
    // SolarCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
